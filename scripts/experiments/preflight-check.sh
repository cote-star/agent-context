...
check_rerun() {
  ...
  if [[ -d "$dir/structured_fresh/.git" ]]; then
    if ! "$AGENT_CONTEXT_BIN" freshness "$dir/structured_fresh" --base-ref "$BASE_REF" >/dev/null 2>&1; then
      echo "FAIL: structured_fresh fails freshness against $BASE_REF"; fail=1
    else
