...
The first command installs the `agent-context` CLI on `PATH`. The second copies the bundled skill to `~/.claude/skills/agent-context/`.

For other agents:

| Agent | Setup |
|---|---|
| Claude Code | `agent-context install-skill --agent claude` (default) |
| Codex | register `~/.local/share/uv/tools/agent-context/lib/python*/site-packages/agent_context/skill/agents/openai.yaml` with your Codex skill registry, or pass `--dest` when installing |
...
| Freshness check via CLI | Run `agent-context freshness . --base-ref origin/main` to check locally |

## Advanced/manual/scripted setup

Once installed via `uv tool install` or `pipx install`, the CLI works from anywhere:
...
If you are working on `agent-context` itself or want to avoid publishing/installing a package:
...
`bin/agent-context` is a thin shim that imports the same module as the installed entry point, so behavior is identical.
