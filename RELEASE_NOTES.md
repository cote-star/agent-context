...
- **PyPI-installable.** New top-level `pyproject.toml` declares the distribution as `agent-context-cli` (stdlib-only, no runtime dependencies). `[project.scripts]` registers `agent-context` as the console entry point — distribution name and binary name differ because the bare `agent-context` slot on PyPI is already taken by an unrelated project. `uv tool install agent-context-cli` (or `pipx install agent-context-cli`) drops the `agent-context` CLI on PATH; `pip install agent-context-cli` works too. The wheel bundles `templates/`, `tools/`, and the full `skills/agent-context/` tree as package data, so the installed CLI is self-sufficient without a clone.
...
- **Auto-release on tag push.** `.github/workflows/release.yml` now builds an sdist + wheel and publishes to PyPI when a `v*` tag is pushed. Authentication uses **PyPI Trusted Publishing (OIDC)** via `pypa/gh-action-pypi-publish` — no long-lived API token in repo secrets. The matching pending publisher must be configured once at https://pypi.org/manage/account/publishing/ for the `agent-context-cli` project (workflow filename `release.yml`). After the PyPI publish job, the existing GitHub Release step attaches sdist + wheel to the tag. Manual fallback for non-CI publish remains `pypi-play publish` from a play-rooted clone.

### Migration

- `git clone … && ~/agent-context/bin/agent-context init` still works unchanged — the shim preserves clone-based usage.
- New recommended path: `uv tool install agent-context-cli` (one command), then `agent-context install-skill claude` (one command). Two commands instead of clone + `cp -r` + PATH wrangling.
