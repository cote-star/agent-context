...
Installability release. `agent-context` is now a real Python package — install via `uv tool install agent-context` (or `pipx install agent-context`) and the CLI is on PATH. A new `install-skill` subcommand copies the bundled skill into the agent's skills directory, so a brand-new user is fully set up in two commands.

### What's new

- **PyPI-installable.** New top-level `pyproject.toml` declares `agent_context` as a stdlib-only package (no runtime dependencies). `[project.scripts]` registers `agent-context` as a console entry point. `uv tool install agent-context` (or `pipx install agent-context`) drops the CLI on PATH; `pip install agent-context` works too. The wheel bundles `templates/`, `tools/`, and the full `skills/agent-context/` tree as package data, so the installed CLI is self-sufficient without a clone.
- **Repo-internal package layout.** CLI code moved from `bin/agent-context` to `agent_context/cli.py` (importable module). `bin/agent-context` is now a thin shim that prepends the repo root to `sys.path` so clone-based use still works the same. Templates, tools, and the bundled skill appear under `agent_context/` as symlinks to canonical locations during development; setuptools resolves them at build time, so no third mirror needs maintaining.
- **`install-skill` subcommand.** `agent-context install-skill --agent claude` copies the bundled skill to `~/.claude/skills/agent-context/`. Supports `--dest PATH`, `--force`, and `--dry-run`. Replaces the manual `cp -r skills/agent-context ~/.claude/skills/` step in the old install flow.
- **Version drift surface added.** `pyproject.toml` is now the fifth pinned location for the version string (alongside the CLI source, root `SKILL.md`, installable `SKILL.md`, and the README badge). `tests/test_version_drift.py` now reads from `agent_context/cli.py` and asserts equality with `pyproject.toml` too.
- **Auto-release on tag push.** `.github/workflows/release.yml` now builds an sdist + wheel and publishes to PyPI when a `v*` tag is pushed. Authentication uses **PyPI Trusted Publishing (OIDC)** via `pypa/gh-action-pypi-publish` — no long-lived API token in repo secrets. The matching pending publisher must be configured once at https://pypi.org/manage/account/publishing/ for the `agent-context` project (workflow filename `release.yml`). After the PyPI publish job, the existing GitHub Release step attaches sdist + wheel to the tag. Manual fallback for non-CI publish remains `pypi-play publish` from a play-rooted clone.

### Migration

- `git clone … && ~/agent-context/bin/agent-context init` still works unchanged — the shim preserves clone-based usage.
- New recommended path: `uv tool install agent-context` (one command), then `agent-context install-skill claude` (one command). Two commands instead of clone + `cp -r` + PATH wrangling.
- For maintainers: the canonical `templates/`, `tools/`, and `skills/agent-context/` directories remain at the repo root and are still the edit targets. The `agent_context/` package contains symlinks to those — do not edit through the package.

### Compatibility

- Existing v0.3.x packs and routing blocks remain valid; the CLI emits the same routing-block content.
- The CLI now self-reports `0.4.0`, matching every other surface for the first time since 0.2.1.
